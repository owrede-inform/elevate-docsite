...
const Code = ({ children, className: classNameProp = '', metaData }) => {
  ...
  // Calculate line count to determine if "Show more" button should be displayed
  useEffect(() => {
    if (children && typeof children === 'string') {
      // Split by newlines and filter out empty trailing lines (similar to removeTrailingEmptyLine logic)
      ...
      setHasMoreThanNineLines(processedLines.length > 9);
    } else {
      setHasMoreThanNineLines(false);
    }
  }, [children]);
